...
exports.loginAdmin = async(req,res)=>{
    if(!(req.body.username || req.body.password)) return res.status(400).json({error: "data not proper formatted"})
    ...
    try {
        ...
        if(admin){
            ...
            if(!validatePass) return res.status(401).json("incorrect password")
        }
...
exports.refreshLoginAdmin = async(req,res)=>{
    ...
    if(!cookie.jwt) return res.status(400).json("no cookie was found here")
    const refreshToken = cookie.jwt

    try {
        const cookieExist = await Admin.exists({refreshToken})
        if(!cookieExist) return res.status(204).json("refreshToken wasn't found")

        // verify jwt cookie
        let admin_detail;
        jwt.verify(refreshToken, process.env.REFRESH_JWT_SECRET,(err,payload)=>{
            if(err) return res.status(401).json("refresh token not present")
            admin_detail = payload
        })

        const newAccessToken = jwt.sign({admin_detail},process.env.ACCESS_JWT_SECRET,{expiresIn: "10m"})
